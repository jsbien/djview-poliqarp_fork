...
QDjViewPrefs::load()
{
  ...
  if (s.contains("modifiersForPrecise"))
    modifiersForPrecise
      = stringToModifiers(s.value("modifiersForPrecise").toString());
...
QDjViewPrefs::save(void)
{
  ...
  s.setValue("modifiersForPrecise", modifiersToString(modifiersForPrecise));
...
QDjViewPrefsDialog::load(QDjView *djview)
{
  ...
  d->ui.keysForPreciseCombo->setValue(prefs->modifiersForPrecise);
  // 4- lens tab
...
QDjViewPrefsDialog::apply()
{
  ...
  prefs->modifiersForPrecise = d->ui.keysForPreciseCombo->value();
  // 4- lens tab
...
QDjViewPrefsDialog::reset()
{
  ...
  d->ui.keysForPreciseCombo->setValue(Qt::ControlModifier|Qt::AltModifier);
  // 4- lens tab
