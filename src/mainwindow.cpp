...
void MainWindow::exportResults()
{
	if (m_exportFilename.isEmpty())
		...
		ui.poliqarpWidget->exportResults(m_exportFilename);
}
...
void MainWindow::documentLoading(const DjVuLink& link)
{
	...
	statusBar()->showMessage(tr("Loading %1...").arg(link.documentPath()));
	m_statusUrl->clear();
	m_statusPage->clear();
}
...
void MainWindow::zoomAction(QAction* action)
{
	if (action->data().toString() == "in")
		...
		ui.djvuWidget->setZoom(action->data().toInt());
}
...
void MainWindow::rotateAction(QAction* action)
{
	if (action->data().toString() == "left")
		...
		ui.djvuWidget->setRotation(action->data().toInt());
}

void MainWindow::showAboutDialog()
{
	QString build = Version::buildNumber() ? tr(" (build %1)").arg(Version::buildText()) : "";
	QString about =
...
void MainWindow::setupActions()
{
	...
	// View menu
	connect(ui.actionViewContinuous, &QAction::toggled, ui.djvuWidget, &QDjVuWidget::setContinuous);
	connect(ui.actionViewSideBySide, &QAction::toggled, ui.djvuWidget, &QDjVuWidget::setSideBySide);
...
void MainWindow::addIndexEntry()
{
	...
	if (url.isValid()) {
		QString hiddenText =
		ui.djvuWidget->getTextForRect(ui.djvuWidget->lastRegion().adjusted(0, 10, 0, -10));
...
void MainWindow::historyChanged(const QString& previous, const QString& next)
{
	...
	if (previous.isEmpty())
		...
		ui.actionPreviousEntry->setText(tr("Previous: %1").arg(previous));

	ui.actionNextEntry->setEnabled(!next.isEmpty());
	if (next.isEmpty())
		ui.actionNextEntry->setText(tr("Next: None"));
	else
		ui.actionNextEntry->setText(tr("Next: %1").arg(next));
}
