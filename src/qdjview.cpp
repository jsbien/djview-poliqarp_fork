...
#define HELP(x,y) { QString s(x); Help h(s); h y; }
  ...
  ml = ml.replace("+"," ");
  mp = prefs->modifiersToString(prefs->modifiersForPrecise);
  mp = mp.replace("+"," ");
  ...
  HELP(tr("<html><b>Document viewing area.</b><br/> "
          ...
          "<li>%4 Left Mouse Button for precise selecting.</li>"
          "</ul></html>").arg(ms).arg(ml).arg(mc).arg(mp),
       >> widget );
...
QDjView::applySaved(Saved *saved)
{
  ...
  tabifyDockWidget(thumbnailDock, samplesDock);
  tabifyDockWidget(thumbnailDock, poliqarpDock);
  thumbnailDock->raise();
  // main saved states
      ...
      if (parse_boolean(key, value, errors, okay) && okay)
        {
          ...
          shapesDock->setVisible(false);
          samplesDock->setVisible(false);
          textDock->setVisible(false);
          poliqarpDock->setVisible(false);
  ...
  if (key == "pages") // readonly
  ...
  else if (key == "passive" || key == "passivestretch")
    {
      if (widget->horizontalScrollBarPolicy() == Qt::ScrollBarAlwaysOff 
          ...
          && !findDock->isVisible() && !shapesDock->isVisible()
          && !samplesDock->isVisible() && !textDock->isVisible() && !poliqarpDock->isVisible()
          && !toolBar->isVisible() && !widget->continuous()
          && !widget->sideBySide() && !widget->keyboardEnabled()
          && !widget->mouseEnabled() && widget->contextMenu() == 0 )
        {
          int z = widget->zoom();
    ...
    { // without position 
      ...
      if (findDock->isVisible())
        what << "find";
      if (shapesDock->isVisible())
        what << "shapes";
      if (samplesDock->isVisible())
        what << "samples";
      if (textDock->isVisible())
        what << "text";
      if (poliqarpDock->isVisible())
        what << "poliqarp";
      if (what.size() > 0)
...
{
  // Main window setup
    setWindowTitle(tr("DjView") + QString("4poliqarp (klf-uw) [build %1]").arg(VER));
  ...
  samplesWidget = new QDjViewSamplesArea(this);
  samplesDock = new QDockWidget(this);
  samplesDock->setObjectName("samplesDock");
  samplesDock->setWindowTitle(tr("Samples"));
  samplesDock->setWidget(samplesWidget);
  samplesDock->installEventFilter(this);
  addDockWidget(Qt::LeftDockWidgetArea, samplesDock);
  shapesWidget = new QDjViewShapes(this, samplesWidget);
  shapesDock = new QDockWidget(this);
  shapesDock->setObjectName("shapesDock");
  shapesDock->setWindowTitle(tr("Shapes"));
  shapesDock->setWidget(shapesWidget);
  shapesDock->installEventFilter(this);
  addDockWidget(Qt::RightDockWidgetArea, shapesDock);
  textWidget = new QDjViewText(this);
  textDock = new QDockWidget(this);
  textDock->setObjectName("textDock");
  textDock->setWindowTitle(tr("Hidden text"));
  textDock->setWidget(textWidget);
  textDock->installEventFilter(this);
  addDockWidget(Qt::BottomDockWidgetArea, textDock);
  textDockTimer = new QTimer(this);
  textDockTimer->setSingleShot(true);
  connect(textDockTimer, SIGNAL(timeout()), textWidget, SLOT(updateTextLabel()));
  poliqarpWidget = new QDjViewPoliqarp(this);
  poliqarpDock = new QDockWidget(this);
  poliqarpDock->setObjectName("poliqarpDock");
  poliqarpDock->setWindowTitle(tr("Poliqarp client"));
  poliqarpDock->setWidget(poliqarpWidget);
  poliqarpDock->installEventFilter(this);
  addDockWidget(Qt::LeftDockWidgetArea, poliqarpDock);
...
QDjView::open(QDjVuDocument *doc, QUrl url)
{
  ...
  widget->setFocus();
  // set title
  setWindowTitle(QString("%1[*] - ").arg(getShortFileName()) + tr("DjView") + QString("4poliqarp (klf-uw) [build %1]").arg(VER));
...
QDjView::open(QUrl url)
{
  ...
  if (docurl.hasFragment()) // remove #sth from url as it does not work with some servers
      docurl.setFragment(QString());
...
QDjView::visibleSideBarTabs()
{
  ...
  if (! findDock->isHidden())
    tabs |= 4;
  if (! shapesDock->isHidden())
    tabs |= 8;
  if (! samplesDock->isHidden())
    tabs |= 16;
  if (! textDock->isHidden())
    tabs |= 32;
  if (! poliqarpDock->isHidden())
    tabs |= 64;
  return tabs;
}
...
QDjView::showSideBar(Qt::DockWidgetAreas areas, int tabs)
{
  ...
  allDocks << thumbnailDock << outlineDock << findDock << shapesDock << samplesDock << textDock << poliqarpDock;
...
QDjView::showSideBar(bool show)
{
  ...
  shapesDock->setVisible(show);
  samplesDock->setVisible(show);
  textDock->setVisible(show);
  poliqarpDock->setVisible(show);
  return true;
}
...
QDjView::pointerPosition(const Position &pos, const PageInfo &info)
{
  ...
  if (textWidget->isVisible())
    {
      textWidget->textDockRect = info.selected;
      textDockTimer->start(25);
    }
}
...
QDjView::performAbout(void)
{
  ...
  QString html = 
    QString::fromUtf8("<html>"
       "<h2>DjVuLibre DjView4poliqarp (klf-uw) %1 [build %3]</h2>%2"
       "<p>"
       "Viewer for DjVu documents<br>"
       "<a href=http://djvulibre.djvuzone.org>"
       "http://djvulibre.djvuzone.org</a><br>"
       "Copyright © 2006-- Léon Bottou.<br>"
       "Copyright © 2010-- Grzegorz Chimosz."
       "</p>"
       "<p align=justify><small>"
       "The modified and extended djview viewer with poliqarp client, "
       "made available by the Formal Linguistics Department of the University of Warsaw, "
       "facilitates searching digitalized texts in the DjVu format. "
       "The modification has been implemented by Grzegorz Chimosz. "
       "The ideas to use DjView as a poliqarp client and others improvements "
       "were developed by Janusz S. Bień. "
       "The work on the djview modification is supported by "
       "the Ministry of Science and Higher Education's grant no. N N519 384036."
       "</small></p>"
       "<p align=justify><small>"
